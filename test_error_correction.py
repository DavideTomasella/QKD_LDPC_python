...
if ~path.exists(fname):
    with open(fname, 'w') as file_output:
        file_output.write("code_n, n_tries,          qber,        f_mean,  c_iters_mean,  n_iters_mean,             R,       s_n,       p_n,   p_n_max,           k_n,   discl_n,           FER \n")

for qber in qber_step+np.arange(qber_start, qber_end, qber_step):
    ...
    with open(fname, 'a') as file_output:
        file_output.write('%d,%10d,%14.4f,%14.8f,%14.8f,%14.8f,%14.8f,%10d,%10d,%10d,%14d,%10d,%14.8f\n' % (n, n_tries,
                                                                                              qber, f_mean, com_iters_mean, n_iters_mean, R, s_n, p_n, p_n_max, k_n, discl_n, FER))
#Pypy